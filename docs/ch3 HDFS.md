...
&emsp;&emsp;**分布式文件系统是管理网络中跨多台计算机存储的文件系统**。该系统架构于网络之上，势必会引入了网络编程的复杂性。因此，分布式文件系统比普通磁盘文件系统更为复杂。例如，如何使文件系统能够容忍节点故障的同时，不会丢失任何数据；在单一节点数据更新的同时通知整个文件系统进行同步更新等等。  
...
> - **分布式文件系统：**也采用了块的概念，文件被分成若干个块进行存储，块是数据读写的基本单元，只不过分布式文件系统的块要比普通文件系统中的块大很多，比如，HDFS默认的一个块的大小是64MB，而且与普通文件系统不同的是，在分布式文件系统中，如果一个文件小于一个数据块的大小，不占用整个数据块的存储空间。设计一个比较大的块，是为了**最小化寻址开销**；但是块的大小也不能太大，会导致MapReduce中的Map任务一次只处理一个块中的数据，如果启动的任务太少，反而会影响并行的速度。
...
&emsp;&emsp;**HDFS（Hadoop Distribute File System）**是大数据领域一种非常可靠的存储系统，它以分布式方式存储超大数据量文件，但它并不适合存储大量的小数据量文件。同时HDFS是Hadoop和其他组件的数据存储层，运行在由价格廉价的商用机器组成的集群上的，而价格低廉的机器发生故障的几率比较高，因此HDFS在设计上采取了多种机制，在硬件故障的情况下保障数据的完整性。总体而言，HDFS要实现以下目标：  
...
&emsp;&emsp;实际上，在系统内部，一个文件会被切分成若干个数据块，这些数据块被分布存储到若干个数据节点上。当客户端需要访问一个文件时，首先把文件名发送给名称节点，名称节点根据文件名找到对应的数据块（一个文件可能包括多个数据块），再根据每个数据块信息找到实际存储在各个数据块的数据节点的位置，并把数据节点位置发送给客户端，最后客户端直接访问这些数据节点获取数据。在整个访问过程中，名称节点并不参与数据的传输。这种设计方式，使得个文件的数据能够在不同的数据节点上实现并发访问，大大提高了数据访问速度。
...
&emsp;&emsp;基于上述前提条件，学习HDFS的进阶命令。

#### 3.5.2.3 实验步骤

##### 1.启动HDFS

&emsp;&emsp;使用**root用户**启动HDFS，若已启动可忽略，命令如下：
```shell
/apps/hadoop/sbin/start-dfs.sh
```
&emsp;&emsp;根据日志显示，分别启动了`NameNode`、`DataNode`、`Secondary NameNode`，执行结果如下：
```log
...
&emsp;&emsp;将本地目录下的`test.txt`文件上传到hdfs文件系统的根目录下，命令如下：  
```shell
hadoop fs -put ./test.txt /
```

&emsp;&emsp;执行完毕后，查看hdfs文件系统的根目录下是否存在`test.txt`文件，命令如下：  
```shell
hadoop fs -ls /
```

##### 4.统计文件数和大小
&emsp;&emsp;统计hdfs文件系统的根目录下的文件数和目录大小，命令如下：  
```shell
hadoop fs -count -h /
```

&emsp;&emsp;其中，三个数值分别表示根目录下文件个数、目录个数和文件的大小，执行结果如下：  
```log
...
##### 5.查找文件
&emsp;&emsp;查找hdfs文件系统的根目录下所有以`txt`结尾的文件，命令如下：  
```shell
...
##### 6.改变文件的副本数
&emsp;&emsp;修改hdfs文件系统根目录下的`test.txt`文件的副本数为1，HDFS默认设置的副本数是3，命令如下：  
```shell
...
##### 7.`test`命令
&emsp;&emsp;检查hdfs文件系统根目录下的`zeno.txt`是否存在，如果存在则返回0，否则返回1，命令如下：  
```shell
hadoop fs -test -e /zeno.txt
```
&emsp;&emsp;再执行如下命令，显示返回值：  
```shell
echo $?
```

&emsp;&emsp;显示结果返回1，表示不存在`zeno.txt`文件，执行结果如下：  
```log
dolphin@tools:~$ echo $?
1
```
...
##### 8.`stat`命令
&emsp;&emsp;展示hdfs文件系统根目录下的`test.txt`文件的统计信息，命令如下：  
```shell
...
&emsp;&emsp;当用户或应用程序通过命令操作hdfs删除某个文件时，这个文件并没有立刻从HDFS中删除。实际上，HDFS会将这个文件重命名转移到`/trash`目录。只要文件还在`/trash`目录中，该文件就可以被迅速地恢复。文件在`/trash`中保存的时间是可配置的，当超过这个时间时，`NameNode`就会将该文件从`DataNode`中删除，删除文件会使得该文件相关的数据块被释放。  
&emsp;&emsp;清理HDFS回收站的所有文件，命令如下：  
```shell
hadoop fs -expunge
```
...
&emsp;&emsp;本章通过两个实验，讲解了在Linux系统中的HDFS文件系统基本命令，通过这些命令可以进一步熟悉HDFS分布式文件系统的使用。  
